...
const Row = ({ label, count, tooltipTitle, translationComponents }: RowProps) => (
    ...
    const rows: RowProps[] = [
        {
            ...
            count: (
                <>
                    <LogsSearchLink response_status={RESPONSE_FILTER.ALL.QUERY}>
                        {formatNumber(numDnsQueries)}
                    </LogsSearchLink>{' '}
                    <span className="counters__value-note">
                        <Trans values={{ count: formatNumber(numCachedDnsQueries) }}>cached_queries_inline</Trans>
                    </span>
                </>
            ),
        ...
        },
        {
            label: 'blocked_by',
            count: (
                <LogsSearchLink response_status={RESPONSE_FILTER.BLOCKED.QUERY}>
                    {formatNumber(numBlockedFiltering)}
                </LogsSearchLink>
            ),
        ...
        },
        {
            ...
            count: (
                <LogsSearchLink response_status={RESPONSE_FILTER.BLOCKED_THREATS.QUERY}>
                    {formatNumber(numReplacedSafebrowsing)}
                </LogsSearchLink>
            ),
        ...
        },
        {
            label: 'stats_adult',
            count: (
                <LogsSearchLink response_status={RESPONSE_FILTER.BLOCKED_ADULT_WEBSITES.QUERY}>
                    {formatNumber(numReplacedParental)}
                </LogsSearchLink>
            ),
        ...
        },
        {
            ...
            count: (
                <LogsSearchLink response_status={RESPONSE_FILTER.SAFE_SEARCH.QUERY}>
                    {formatNumber(numReplacedSafesearch)}
                </LogsSearchLink>
            ),
